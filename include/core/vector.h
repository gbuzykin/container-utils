...
class vector : public std::allocator_traits<Alloc>::template rebind_alloc<Ty> {
 ...
 public:
    ...
    vector() NOEXCEPT_IF(std::is_nothrow_default_constructible<alloc_type>::value) {}
    explicit vector(const allocator_type& alloc) NOEXCEPT : alloc_type(alloc) {}
    explicit vector(size_type sz, const allocator_type& alloc = allocator_type()) : vector(alloc) {
        tidy_invoke([&]() { init_default(sz); });
    }

    vector(size_type sz, const value_type& val, const allocator_type& alloc = allocator_type()) : vector(alloc) {
        tidy_invoke([&]() { init(sz, const_value(val)); });
    }

    vector(std::initializer_list<value_type> l, const allocator_type& alloc = allocator_type()) : vector(alloc) {
    ...
    vector(InputIt first, InputIt last, const allocator_type& alloc = allocator_type()) : vector(alloc) {
    ...
    iterator insert(const_iterator pos, size_type count, const value_type& val) {
        auto p = insert_const(to_ptr(pos, *this), count, val, std::is_copy_assignable<Ty>());
        return iterator(p, begin_, end_);
    }

    template<typename InputIt, typename = std::enable_if_t<is_input_iterator<InputIt>::value>>
    iterator insert(const_iterator pos, InputIt first, InputIt last) {
        auto p = insert_from_range(to_ptr(pos, *this), first, last, is_random_access_iterator<InputIt>(),
                                   std::is_assignable<Ty&, decltype(*std::declval<InputIt>())>());
        return iterator(p, begin_, end_);
    }

    iterator insert(const_iterator pos, std::initializer_list<value_type> l) {
        auto p = insert_impl(to_ptr(pos, *this), l.size(), l.begin(), std::is_copy_assignable<Ty>());
        return iterator(p, begin_, end_);
    }
    ...
    iterator emplace(const_iterator pos, Args&&... args) {
        auto p = emplace_impl(to_ptr(pos, *this), std::forward<Args>(args)...);
        return iterator(p, begin_, end_);
    }
 ...
 private:
    ...
    static pointer to_ptr(const_iterator it, const vector& v) {
        auto p = std::addressof(const_cast<reference>(*it.ptr(v.begin_, v.end_)));
        assert((v.begin_ <= p) && (p <= v.end_));
        return p;
    }
...
bool operator==(const vector<Ty, Alloc>& lh, const vector<Ty, Alloc>& rh) {
