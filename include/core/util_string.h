...
class basic_string_view {
 public:
    using traits_type = std::char_traits<Ty>;
    using value_type = Ty;
    using pointer = Ty*;
    ...
    basic_string_view(const Ty* s, size_type count) : begin_(s), size_(count) {}
    basic_string_view(const Ty* s) : begin_(s) {
        for (; *s; ++s, ++size_) {}
    }
    basic_string_view(const basic_string<Ty>& s) : basic_string_view(s.data(), s.size()) {}

    size_type size() const { return size_; }
    size_type length() const { return size_; }
    bool empty() const { return size_ == 0; }

    explicit operator basic_string<Ty>() const { return basic_string<Ty>(begin_, size_); }
    ...
    size_type find(Ty ch, size_type pos = 0) const;
    size_type find(basic_string_view s, size_type pos = 0) const;
    size_type rfind(Ty ch, size_type pos = npos) const;
    ...
    friend basic_string<Ty>& operator+=(basic_string<Ty>& lhs, basic_string_view rhs) {
        lhs.append(rhs.data(), rhs.size());
        return lhs;
    }

 private:
    const Ty* begin_ = nullptr;
    size_t size_ = 0;
};

template<typename Ty>
int basic_string_view<Ty>::compare(basic_string_view<Ty> s) const {
...
template<typename Ty>
auto basic_string_view<Ty>::find(Ty ch, size_type pos) const -> size_type {
...
template<typename Ty>
auto basic_string_view<Ty>::find(basic_string_view s, size_type pos) const -> size_type {
...
template<typename Ty>
auto basic_string_view<Ty>::rfind(Ty ch, size_type pos) const -> size_type {
...
template<typename Ty>
auto basic_string_view<Ty>::rfind(basic_string_view s, size_type pos) const -> size_type {
...
#    if defined(_MSC_VER)
template<typename Ty>
struct hash<basic_string_view<Ty>> {
    size_t operator()(basic_string_view<Ty> s) const { return _Hash_seq((const unsigned char*)s.data(), s.size()); }
};
#    else   // !defined(_MSC_VER)
template<typename Ty>
struct hash<basic_string_view<Ty>> {
    size_t operator()(basic_string_view<Ty> s) const {
        return std::hash<std::basic_string<Ty>>{}(static_cast<std::basic_string<Ty>>(s));
    }
};
#    endif  // _MSC_VER
}  // namespace std
...
template<typename Ty, typename = std::void_t<typename impl::string_finder<Ty>::is_finder>>
impl::string_finder<Ty> sfind(const Ty& v) {
    return impl::string_finder<Ty>(v);
}

template<typename Ty, typename = std::void_t<typename impl::reversed_string_finder<Ty>::is_reversed_finder>>
impl::reversed_string_finder<Ty> rsfind(const Ty& v) {
    return impl::reversed_string_finder<Ty>(v);
}

inline impl::string_finder<char> sfind(char ch) { return impl::string_finder<char>(ch); }
inline impl::reversed_string_finder<char> rsfind(char ch) { return impl::reversed_string_finder<char>(ch); }
inline impl::string_finder<std::string_view> sfind(std::string_view s) {
    return impl::string_finder<std::string_view>(s);
}
inline impl::reversed_string_finder<std::string_view> rsfind(std::string_view s) {
    return impl::reversed_string_finder<std::string_view>(s);
}
