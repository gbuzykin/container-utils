...
class rbtree : public rbtree_base<NodeTy, Alloc, Comp> {
 protected:
 ...
 public:
    ...
    rbtree() = default;
    explicit rbtree(const allocator_type& alloc) NOEXCEPT_IF(std::is_nothrow_default_constructible<key_compare>::value)
        : super(alloc) {}
    explicit rbtree(const key_compare& comp, const allocator_type& alloc) : super(comp, alloc) {}
    rbtree(const rbtree& other, const allocator_type& alloc) : super(other, alloc) {}
    rbtree(rbtree&& other, const allocator_type& alloc) : super(std::move(other), alloc) {}

#if __cplusplus < 201703L
    rbtree(const rbtree&) = default;
    rbtree& operator=(const rbtree&) = default;
    rbtree(rbtree&& other) : super(std::move(other)) {}
    ...
    iterator emplace_hint(const_iterator hint, Args&&... args) {
        typename super::delete_guard_t g(*this, super::helpers::new_node(*this, std::forward<Args>(args)...));
        auto result = rbtree_find_insert_unique_pos<node_t>(std::addressof(this->head_), this->to_ptr(hint, *this),
...
template<typename NodeTy, typename Alloc, typename Comp>
template<typename InputIt>
void rbtree<NodeTy, Alloc, Comp>::assign_impl(InputIt first, InputIt last, std::true_type) {
    assert(super::check_iterator_range(first, last, is_random_access_iterator<InputIt>()));
    if (this->size_) {
...
template<typename NodeTy, typename Alloc, typename Comp>
template<typename InputIt>
void rbtree<NodeTy, Alloc, Comp>::assign_impl(InputIt first, InputIt last, std::false_type) {
    assert(super::check_iterator_range(first, last, is_random_access_iterator<InputIt>()));
    if (this->size_) {
        typename super::temp_chain_t tmp(std::move(*this));
...
template<typename NodeTy, typename Alloc, typename Comp>
template<typename Comp2>
void rbtree<NodeTy, Alloc, Comp>::merge_impl(rbtree_base<NodeTy, Alloc, Comp2>&& other) {
